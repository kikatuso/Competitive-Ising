...
class mf_ising_system:
    ...
    def __init__(self,graph,background_field,lr_1,lr_2,fixed_point_iter=int(5*1e4),init_mag='random',fp_tol_fac=1e-5,
        iim_iter=5000,iim_tol_fac=1e-3,momentum=0.4,optimiser_type='sgdm'):
    ...
    def positive_agent(self,mag_i,it,pos_budget,beta):
        ...
        gradient = mag_i_grad*control_field
        if self.optimiser_type=='sgd':
            ...
            control_field_update = control_field + self.sgdm(gradient,self.changes_pos,'pos',it)
    ...
    def negative_agent(self,mag_i,it,neg_budget,beta):
        ...
        if self.optimiser_type=='sgd':
            ...
            control_field_update = control_field -  self.sgdm(gradient,self.changes_neg,'neg',it)
    ...
    def sgdm(self,grad,changes,typ,it):
        if typ=='pos':
        ...
        new_change = step_size * grad + self.momentum * changes[-1]
        getattr(self,name).append(new_change)
        return new_change
    
    
    def second_partial_dffs(self,mag_ii,tot_field,control_pos,control_neg,beta,a=1e-5):
        ' estimating 2nd partial derivative with respect to each agents external field' 
        grad0 = self.mag_grad(beta,mag_ii)
        
        update = np.array([a*field if field!=0 else a for field in tot_field])
        ...
        curv_player_pos = second_total_grad*control_pos**2+grad0
        curv_player_neg = second_total_grad*control_neg**2+grad0
    ...
    def MF_IIM(self,pos_budget,neg_budget,beta,init_alloc='random'):
        ...
        for it in range(self.iim_iter):
            ...
            second_dffs=self.second_partial_dffs(mag_ii,tot_field,control_pos,control_neg,beta) 
